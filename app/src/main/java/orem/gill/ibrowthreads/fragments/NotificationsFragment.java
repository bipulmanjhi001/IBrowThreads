...
import orem.gill.ibrowthreads.adapters.NotificationsAdapter;
...
public class NotificationsFragment extends Fragment {
    ...
    public void apiGetNotifications(){
        showDialog(getString(R.string.loading));
        Call<ResponseBody> call=Utils.requestApiDefault().requestJson_simple(GeneralValues.URL_GET_NOTIFICATIONS);
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                try {
                    ...
                    if (res != null && res.equals("1")) {
                        ...
                        ArrayList<NotificationsPOJO> arr=new ArrayList<NotificationsPOJO>();
                        for(int i=0;i<dataArr.length();i++){
                            JSONObject item=dataArr.getJSONObject(i);
                            arr.add(new NotificationsPOJO(item.optString("id"),
                                    item.optString(""),
                                    item.optString("notification"),
                                    item.optString("creation_date")));
                        }
                        if(isAdded()){
                            NotificationsAdapter adapter=new NotificationsAdapter(mContext,arr);
