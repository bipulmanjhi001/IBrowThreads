...
import orem.gill.ibrowthreads.adapters.OffersAdapter;
...
public class OffersFragment extends Fragment {
    ...
    public void apiGetOffers(){
        ...
        Call<ResponseBody> call=Utils.requestApiDefault().requestJson_simple(GeneralValues.URL_GET_OFFERS);
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                try {
                    ...
                    if (res != null && res.equals("1")) {
                        ...
                        ArrayList<OffersPOJO> arr=new ArrayList<OffersPOJO>();
                        for(int i=0;i<dataArr.length();i++){
                            JSONObject item=dataArr.getJSONObject(i);
                            arr.add(new OffersPOJO(item.optString("id"),
                                    item.optString("name"),
                                    item.optString("description"),
                                    item.optString("stamps"),
                                    item.optString("image")));
                        }
                        if(isAdded()){
                            OffersAdapter adapter=new OffersAdapter(mContext,arr);
